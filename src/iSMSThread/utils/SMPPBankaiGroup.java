...
public class SMPPBankaiGroup {
	...
	String ipAddress ="212.83.163.254";// "localhost"; // 41.244.255.6
	...
	String systemId = "IplansCD";//"smppclient1"; // KAKOTEL
	...
	String password = "jtkfaetb";//"password"; // kak@123
	 ...
	 */
	public SMPPBankaiGroup() throws IOException {
	...
	public static boolean main(String args[]) {
		...
		if ((dest == null) || (message == null)) {
			System.out.println("Usage: SMPPBankaigroup -dest <dest number on international format> -message <the message, "
		...
		SMPPBankaiGroup smppSender = null;
		try {
			smppSender = new SMPPBankaiGroup();
		} catch (IOException e) {
			System.out.println("Exception initialising SMPPBankaiGroup " + e);
		}

		System.out.println("Sending: \"" + message + "\" to " + dest);
		if (smppSender != null) {
			System.err.println("before bind");
			smppSender.bind();
			System.err.println("after bind");
			if (smppSender.bound) {
				...
				System.err.println("before unbind");
				smppSender.unbind();
				System.err.println("after unbind");
				return true;
			}
		}
		return false;
